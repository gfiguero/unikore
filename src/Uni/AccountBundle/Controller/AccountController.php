...
use Uni\AccountBundle\Form\AccountType;
...
class AccountController extends Controller
{
    ...
    public function indexAction(Request $request)
    {
        ...
        if($sort) $accounts = $em->getRepository('UniAdminBundle:Account')->findBy(array(), array($sort => $direction));
        else $accounts = $em->getRepository('UniAdminBundle:Account')->findAll();
        ...
        return $this->render('UniAccountBundle:Account:index.html.twig', array(
            'accounts' => $accounts,
            'direction' => $direction,
            'sort' => $sort,
    ...
    public function newAction(Request $request)
    {
        ...
        if ($newForm->isSubmitted()) {
            if($newForm->isValid()) {
                ...
                return $this->redirect($this->generateUrl('account_account_index'));
            }
        }

        return $this->render('UniAccountBundle:Account:new.html.twig', array(
    ...
    private function createNewForm(Account $account)
    {
        return $this->createForm(new AccountType(), $account, array(
            'action' => $this->generateUrl('account_account_new'),
        ));
    }
    ...
    public function showAction(Account $account)
    {
        ...
        return $this->render('UniAccountBundle:Account:show.html.twig', array(
            'account' => $account,
    ...
    public function editAction(Request $request, Account $account)
    {
        ...
        return $this->render('UniAccountBundle:Account:edit.html.twig', array(
            'account' => $account,
            'editForm' => $editForm->createView(),
            'deleteForm' => $deleteForm->createView(),
        ));
    ...
    {
        return $this->createForm(new AccountType(), $account, array(
            'action' => $this->generateUrl('account_account_edit', array('id' => $account->getId())),
        ));
    }
    ...
    private function createDeleteForm(Account $account)
    {
        return $this->createFormBuilder()
            ->setAction($this->generateUrl('account_account_delete', array('id' => $account->getId())))
            ->setMethod('DELETE')
            ->getForm()
        ;
    }

...
use Uni\AdminBundle\Entity\Seller;
use Uni\OfferBundle\Form\SellerType;
...
class SellerController extends Controller
{
    ...
    public function indexAction(Request $request)
    {
        ...
        return $this->render('UniOfferBundle:Seller:index.html.twig', array(
            'sellers' => $sellers,
            'direction' => $direction,
            'sort' => $sort,
    ...
    public function newAction(Request $request)
    {
        ...
        if ($newForm->isSubmitted()) {
            if($newForm->isValid()) {
                ...
                return $this->redirect($this->generateUrl('offer_seller_index'));
            }
        }

        return $this->render('UniOfferBundle:Seller:new.html.twig', array(
    ...
    private function createNewForm(Seller $seller)
    {
        return $this->createForm(new SellerType(), $seller, array(
            'action' => $this->generateUrl('offer_seller_new'),
        ));
    }
    ...
    public function showAction(Seller $seller)
    {
        $user = $this->getUser();
        $account = $user->getAccount();
        if ($account != $seller->getAccount()) return $this->redirect($this->generateUrl('offer_seller_index'));

        $editForm = $this->createEditForm($seller);
        $deleteForm = $this->createDeleteForm($seller);

        return $this->render('UniOfferBundle:Seller:show.html.twig', array(
            'seller' => $seller,
    ...
    public function editAction(Request $request, Seller $seller)
    {
        ...
        return $this->render('UniOfferBundle:Seller:edit.html.twig', array(
            'seller' => $seller,
            'editForm' => $editForm->createView(),
            'deleteForm' => $deleteForm->createView(),
        ));
    ...
    {
        return $this->createForm(new SellerType(), $seller, array(
            'action' => $this->generateUrl('offer_seller_edit', array('id' => $seller->getId())),
        ));
    }
    ...
    private function createDeleteForm(Seller $seller)
    {
        return $this->createFormBuilder()
            ->setAction($this->generateUrl('offer_seller_delete', array('id' => $seller->getId())))
            ->setMethod('DELETE')
            ->getForm()
        ;
    }

...
use Uni\OfferBundle\Form\NoteType;
...
class NoteController extends Controller
{
    ...
    public function indexAction(Request $request)
    {
        ...
        return $this->render('UniOfferBundle:Note:index.html.twig', array(
            'notes' => $notes,
            'direction' => $direction,
            'sort' => $sort,
    ...
    public function newAction(Request $request)
    {
        ...
        if ($newForm->isSubmitted()) {
            if($newForm->isValid()) {
                ...
                return $this->redirect($this->generateUrl('offer_note_index'));
            }
        }

        return $this->render('UniOfferBundle:Note:new.html.twig', array(
    ...
    private function createNewForm(Note $note)
    {
        return $this->createForm(new NoteType(), $note, array(
            'action' => $this->generateUrl('offer_note_new'),
        ));
    }
    ...
    public function showAction(Note $note)
    {
        $user = $this->getUser();
        $account = $user->getAccount();
        if ($account != $note->getAccount()) return $this->redirect($this->generateUrl('offer_note_index'));

        $editForm = $this->createEditForm($note);
        $deleteForm = $this->createDeleteForm($note);

        return $this->render('UniOfferBundle:Note:show.html.twig', array(
            'note' => $note,
    ...
    public function editAction(Request $request, Note $note)
    {
        ...
        return $this->render('UniOfferBundle:Note:edit.html.twig', array(
            'note' => $note,
            'editForm' => $editForm->createView(),
            'deleteForm' => $deleteForm->createView(),
        ));
    ...
    {
        return $this->createForm(new NoteType(), $note, array(
            'action' => $this->generateUrl('offer_note_edit', array('id' => $note->getId())),
        ));
    }
    ...
    private function createDeleteForm(Note $note)
    {
        return $this->createFormBuilder()
            ->setAction($this->generateUrl('offer_note_delete', array('id' => $note->getId())))
            ->setMethod('DELETE')
            ->getForm()
        ;
    }

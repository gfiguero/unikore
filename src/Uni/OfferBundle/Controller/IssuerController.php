...
use Uni\AdminBundle\Entity\Issuer;
use Uni\OfferBundle\Form\IssuerType;
...
class IssuerController extends Controller
{
    ...
    public function indexAction(Request $request)
    {
        ...
        return $this->render('UniOfferBundle:Issuer:index.html.twig', array(
            'issuers' => $issuers,
            'direction' => $direction,
            'sort' => $sort,
    ...
    public function newAction(Request $request)
    {
        ...
        if ($newForm->isSubmitted()) {
            if($newForm->isValid()) {
                ...
                return $this->redirect($this->generateUrl('offer_issuer_index'));
            }
        }

        return $this->render('UniOfferBundle:Issuer:new.html.twig', array(
    ...
    private function createNewForm(Issuer $issuer)
    {
        return $this->createForm(new IssuerType(), $issuer, array(
            'action' => $this->generateUrl('offer_issuer_new'),
        ));
    }
    ...
    public function showAction(Issuer $issuer)
    {
        $user = $this->getUser();
        $account = $user->getAccount();
        if ($account != $issuer->getAccount()) return $this->redirect($this->generateUrl('offer_issuer_index'));

        $editForm = $this->createEditForm($issuer);
        $deleteForm = $this->createDeleteForm($issuer);

        return $this->render('UniOfferBundle:Issuer:show.html.twig', array(
            'issuer' => $issuer,
    ...
    public function editAction(Request $request, Issuer $issuer)
    {
        ...
        return $this->render('UniOfferBundle:Issuer:edit.html.twig', array(
            'issuer' => $issuer,
            'editForm' => $editForm->createView(),
            'deleteForm' => $deleteForm->createView(),
        ));
    ...
    {
        return $this->createForm(new IssuerType(), $issuer, array(
            'action' => $this->generateUrl('offer_issuer_edit', array('id' => $issuer->getId())),
        ));
    }
    ...
    private function createDeleteForm(Issuer $issuer)
    {
        return $this->createFormBuilder()
            ->setAction($this->generateUrl('offer_issuer_delete', array('id' => $issuer->getId())))
            ->setMethod('DELETE')
            ->getForm()
        ;
    }
